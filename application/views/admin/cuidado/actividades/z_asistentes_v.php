...
var actividadAsistentesApp = new Vue({
    ...
    methods: {
        getList: function(){
            ...
            formValues.append('condition', 'related_1 = ' + this.session.id + ' AND type_id = 100023')
        ...
        addStudent: function(userKey){
            ...
            formValues.append('user_id', this.users[userKey].id)
            formValues.append('type_id', 100023)
            formValues.append('related_1', this.session.id)
            formValues.append('text_1', this.users[userKey].display_name)
            formValues.append('text_2', this.users[userKey].document_number)
            axios.post(URL_API + 'users/save_meta/related_1', formValues)
            .then(response => {
        ...
        delete_element: function(){
            var metaId = this.students[this.userKey].id
            var userId = this.students[this.userKey].user_id
            axios.get(URL_API + 'users/delete_meta/' + userId + '/' + metaId)
            .then(response => {
